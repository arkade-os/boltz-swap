...
describe("Sloppy Network", () => {
    ...
    it(
        "should handle sloppy network conditions",
        { timeout: 100_000 },
        async () => {
                ...
                return async () => {
                    ...
                    const finalCheckpoints = await Promise.all(
                        signedCheckpointTxs.map(async (c, idx) => {
                            ...
                            const signedCheckpoint = await identity.sign(tx, [
                                0,
                            ]);
            ...
            const onStatusUpdate = async (
                ...
                switch (status) {
                    ...
                    case "invoice.settled": {
                        ...
                        const swapStatus =
                            await swapProvider.getReverseSwapTxId(
                                pendingSwap.id
                            );
            ...
            const monitorSwap = (
                ...
                return new Promise((resolve, reject) => {
                    ...
                    const connectionTimeout = setTimeout(() => {
                        webSocket.close();
                        reject(
                            new NetworkError("WebSocket connection timeout")
                        );
                    ...
                    webSocket.onmessage = async (rawMsg) => {
                        ...
                        if (msg.args[0].error) {
                            webSocket.close();
                            reject(
                                new SwapError({ message: msg.args[0].error })
                            );
                        }
